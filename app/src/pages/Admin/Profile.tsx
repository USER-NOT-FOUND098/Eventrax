    ...
    const fetchStats = async () => {
        try {
            ...
            setStats({
                ...
                totalReports: 0,
                activeUsers: usersRes.data?.filter((u: any) => u.status === 'active')?.length || 0,
            });
        } catch (error) {
                    ...
                    <div className="max-w-4xl mx-auto">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                            ...
                                            <TabsContent value="basic" className="space-y-4">
                                                <div className="flex items-center gap-4">
                                                    <div className="relative">
                                                        ...
                                                        {editMode && (
                                                            <div className="absolute -bottom-2 -right-2">
                                                                <button onClick={() => setShowAvatarDialog(true)} className="p-1 bg-red-600 text-white rounded-full hover:bg-red-700">
                                                    ...
                                                    <div className="flex-1">
                                                        ...
                                                        <Badge className={cn("ml-2", getRoleColor(profile?.role || ''))}>Administrator</Badge>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                    <div>
                                                        ...
                                                        <Input id="name" value={editMode ? formData.name || '' : profile?.name || ''} onChange={(e) => handleInputChange('name', e.target.value)} disabled={!editMode} className={editMode ? 'border-red-300' : ''} />
                                                    </div>
                                                    <div>
                                                    ...
                                                    <div>
                                                        ...
                                                        <Input id="phone" value={editMode ? formData.phone || '' : profile?.phone || ''} onChange={(e) => handleInputChange('phone', e.target.value)} disabled={!editMode} placeholder="+1 (555) 123-4567" className={editMode ? 'border-red-300' : ''} />
                                                    </div>
                                                    <div>
                                                        <Label htmlFor="institution">Organization</Label>
                                                        <Input id="institution" value={editMode ? formData.institution || '' : profile?.institution || ''} onChange={(e) => handleInputChange('institution', e.target.value)} disabled={!editMode} className={editMode ? 'border-red-300' : ''} />
                                                    </div>
                                                </div>
                                            </TabsContent>
                            ...
                            <div className="lg:col-span-1">
                                <Card>
                                    <CardHeader>
                                        <CardTitle className="flex items-center gap-2"><Activity className="h-5 w-5" />Admin Stats</CardTitle>
                                    </CardHeader>
                                    <CardContent>
                                        <div className="space-y-4">
                                            <div className="flex items-center justify-between p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                                                ...
                                                <span className="text-lg font-bold text-blue-600 dark:text-blue-400">{stats.totalUsers}</span>
                                            </div>
                                            <div className="flex items-center justify-between p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                                                ...
                                                <span className="text-lg font-bold text-purple-600 dark:text-purple-400">{stats.totalEvents}</span>
                                            </div>
                                            <div className="flex items-center justify-between p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
                                                ...
                                                <span className="text-lg font-bold text-green-600 dark:text-green-400">{stats.activeUsers}</span>
                                            </div>
                                        </div>
                                    </CardContent>
                                </Card>
                    ...
                    <div className="grid grid-cols-6 gap-3 py-4 max-h-96 overflow-y-auto">
                        {avatarTemplates.map((avatar, index) => (
                            <button key={index} onClick={() => handleAvatarSelect(avatar)} className="relative group">
                                <Avatar className="h-16 w-16 mx-auto border-2 border-gray-200 group-hover:border-red-500 transition-colors">
                                    <AvatarImage src={avatar} alt={`Avatar ${index + 1}`} />
                                    <AvatarFallback>{String.fromCharCode(65 + index)}</AvatarFallback>
                                </Avatar>
                                {(avatarPreview || profile?.avatar) === avatar && (
                                    <div className="absolute inset-0 flex items-center justify-center">
                                        <div className="bg-red-600 text-white rounded-full p-1"><CheckCircle className="h-4 w-4" /></div>
                                    </div>
                                )}
                            </button>
                        ))}
